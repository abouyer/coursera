...
for i=1:m,
  ...
  y_vect(y(i)) = 1;
  J = J + 1 / m * sum(- y_vect' .* log(a_3(i,:)) -
        (1 - y_vect)' .* log(1 - a_3(i,:)));
end

Theta1_reg = Theta1(:, 2:size(Theta1, 2));
Theta2_reg = Theta2(:, 2:size(Theta2, 2));

J = J + lambda * (sum(sum(power(Theta1_reg, 2))) +
      sum(sum(power(Theta2_reg, 2)))) / 2 / m;
...
delta_cap_1 = zeros(size(Theta1));
delta_cap_2 = zeros(size(Theta2));
